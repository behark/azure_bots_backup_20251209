...
class FibReversalBot:
    ...
    def _analyze_symbol(self, symbol: str, timeframe: str, speed: str) -> Optional[FibReversalSignal]:
        ...
        if TPSLCalculator is not None and CalculationMethod is not None:
            ...
            calculator = TPSLCalculator(min_risk_reward=min_rr, max_sl_percent=None)
            ...
            if levels.is_valid:
                ...
                if direction == "BULLISH":
                    assert sl < entry, f"FibReversal BULLISH: SL {sl} should be < entry {entry}"
                    assert tp1 > entry, f"FibReversal BULLISH: TP1 {tp1} should be > entry {entry}"
                else:  # BEARISH
                    assert sl > entry, f"FibReversal BEARISH: SL {sl} should be > entry {entry}"
                    assert tp1 < entry, f"FibReversal BEARISH: TP1 {tp1} should be < entry {entry}"
                
                logger.debug("%s %s FibReversal | Entry: %.6f | SL: %.6f | TP1: %.6f | TP2: %.6f | Speed: %s",
                           symbol, direction, entry, sl, tp1, tp2, speed)
            else:
            ...
            if direction == "BULLISH":
                assert sl < entry, f"FibReversal BULLISH fallback: SL {sl} should be < entry {entry}"
                assert tp1 > entry, f"FibReversal BULLISH fallback: TP1 {tp1} should be > entry {entry}"
            else:  # BEARISH
                assert sl > entry, f"FibReversal BEARISH fallback: SL {sl} should be > entry {entry}"
                assert tp1 < entry, f"FibReversal BEARISH fallback: TP1 {tp1} should be < entry {entry}"
